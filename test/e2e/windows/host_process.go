...
var _ = SIGDescribe("[Feature:WindowsHostProcessContainers] [MinimumKubeletVersion:1.22] HostProcess containers", func() {
	...
	ginkgo.It("container stats validation", func() {
		...
		wait.Poll(2*time.Second, timeout, func() (bool, error) {
			return ensurePodsStatsOnNode(f.ClientSet, f.Namespace.Name, targetNode.Name, &statsChecked)
		})
...
func ensurePodsStatsOnNode(c clientset.Interface, namespace, nodeName string, statsChecked *bool) (bool, error) {
	nodeStats, err := e2ekubelet.GetStatsSummary(c, nodeName)
