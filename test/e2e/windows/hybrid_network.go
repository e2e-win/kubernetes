...
var _ = SIGDescribe("Hybrid cluster network", func() {
	...
	ginkgo.Context("for all supported CNIs", func() {
		...
		ginkgo.It("should have stable networking for Linux and Windows pods", func() {
			...
			assertConsistentConnectivity(f, linuxPod.ObjectMeta.Name, linuxOS, linuxCheck(windowsPod.Status.PodIP, 80), internalMaxTries)

			ginkgo.By("checking connectivity from Windows to Linux")
			assertConsistentConnectivity(f, windowsPod.ObjectMeta.Name, windowsOS, windowsCheck(linuxPod.Status.PodIP), internalMaxTries)
		...
		ginkgo.It("should provide Internet connection for Linux containers using DNS [Feature:Networking-DNS]", func() {
			...
			assertConsistentConnectivity(f, linuxPod.ObjectMeta.Name, linuxOS, linuxCheck("8.8.8.8", 53), externalMaxTries)
		})
		...
		ginkgo.It("should provide Internet connection for Windows containers using DNS [Feature:Networking-DNS]", func() {
			...
			assertConsistentConnectivity(f, windowsPod.ObjectMeta.Name, windowsOS, windowsCheck("www.google.com"), externalMaxTries)
		})
...
func assertConsistentConnectivity(f *framework.Framework, podName string, os string, cmd []string, maxTries int) {
	...
	gomega.Eventually(connChecker, warmUpDuration, pollInterval).ShouldNot(gomega.HaveOccurred())
